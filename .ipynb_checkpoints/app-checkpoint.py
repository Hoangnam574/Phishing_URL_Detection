...
from external_api_fetcher import extract_features_from_apis
from utils import remove_vietnamese_diacritics, SUSPICIOUS_KEYWORDS, strip_scheme_www, normalize_url
from title_feature_extractor import TitleFeatureExtractor
...
def calculate_risk_score(ai_prediction: int, api_features: dict, url: str = "", title: str = "", domain_ok: bool = True) -> tuple[int, int]:
    ...
    url_hits   = sum(1 for w in keywords if w in url_clean)
    title_hits = sum(1 for w in keywords if w in title_clean)
    ...
    # --- Không AI, không API, chỉ keyword ---
    ...
    if is_ip_url:
        ...
        if re.search(r"(\d)\1{1,}", dom):  # số lặp
            score += 1
        if len(set(re.findall(r"\d", dom))) >= 3:
            score += 1

    # --- Domain không phân giải ---
    if not domain_ok:
        score += 2

    # --- Nếu AI chưa cảnh báo mà điểm >= 7 ---
